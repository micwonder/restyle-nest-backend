...
export const s3Service_converted = async (path: string, file: any) => {
  ...
  // console.log('aws_bucket', process.env.AWS_S3_BUCKET_NAME, process.env.process.env.AWS_S3_BUCKET_NAME_REGION);
  return await s3_upload(
    readData,
    process.env.AWS_S3_BUCKET_NAME,
    file.filename,
...
export const s3VideoService_converted = async (path: string, file: any) => {
  const readData = await fs.readFile(`${path}`);
  console.log('fileinfo===================>>>>', file);
  console.log('readdata===================>>>>', readData);
  // console.log('aws_bucket', process.env.AWS_S3_BUCKET_NAME, process.env.process.env.AWS_S3_BUCKET_NAME_REGION);
